...
const connectionString = 'mongodb+srv://niteshnishanth8:KuramaNaruto1@mongoose.syl2ubr.mongodb.net/Nitesh?retryWrites=true&w=majority';
...
let db = null;
connectToDb();
const Store = mongoose.model('Nitesh', storeSchema, 'Nitesh');
...
passport.use(new LocalStrategy(
    ...
    async function(username, password, done) {
      try {
        const user = await User.findOne({ username: username }).maxTimeMS(30000).catch((error)=>{console.log(error)})
...
async function connectToDb() {
  ...
  try {
    db = await mongoose.connect(connectionString, { useNewUrlParser: true, useUnifiedTopology: true, connectTimeoutMS: 30000 });
